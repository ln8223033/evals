...
(function($) {
	...
	$.addFlex = function(t, p) {
		...
		p = $.extend({
			...
			rp : 15, // 每页默认的结果数
			rpOptions : [5,10, 15, 20, 25, 50, 100, 200],// 可选择设定的每页结果数
			title : false,// 是否包含标题
			...
			onRowDblclick: false,//自定义双击事件
            onRowClick: false//自定义单击事件
		  }, p);
		...
		var g = {
			...
			rePosDrag : function() {
				...
				if($.browser.msie && $.browser.version >= 8.0){//解决ie 8 兼容 edit by huitang
					$('thead tr:first th:visible', this.hDiv).each(function() {
				    	cdpos = cdpos + cdleft + cdpad;
				    	var tmp = $('thead tr:first th:visible', g.hDiv);
				    	var n = $(tmp).index(this);
				    	var currentDisplay = tmp[n].style.display;
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function(i, row) {
						    ...
						    $('thead tr:first th', g.hDiv).each(function() {
							        ...
							        td.align = this.align;
							        td.innerHTML = ( tdVal[idx]==null?"":tdVal[idx]);
			...
			addRowProp : function() {
				$('tbody tr', g.bDiv).each(function() {
					    $(this).click(function(e) {
						    ...
						    // 添加多选框
						    if (p.checkbox) {
							    if ($(this).hasClass('trSelected')) {$(this).find('input')[0].checked = true;} 
							...
							if (p.onRowClick) p.onRowClick($(rowData));
